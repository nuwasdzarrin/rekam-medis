...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $rekams = Rekam::query()->select(['id','dokter_id', 'biaya_tindakan', 'biaya_resep', 'diskon', 'created_at', 'cara_bayar', 'platform_pembayaran'])
            ->whereBetween('created_at', [$from, $to]);
        if ($request->filled('doctor_id')) $rekams = $rekams->where('dokter_id', $request->doctor_id);
        $rekams = $rekams->with('dokter:id,nama')->get();
